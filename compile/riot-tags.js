...
riot.tag2('project', '<div class="image" riot-style="background-image: url(\'{profile_picture}\');"> </div> <section class="{this.contentClass()}"> <h1 if="{!opts.donate_button}"><unsafe-html html="{record.title}"></unsafe-html></h1> <div class="limited-width"> <div if="{!record.donations_prohibited && !opts.donate_button}" class="project-values"> <div class="progress-bar" if="{record.progress_percentage && !opts.donate_button}"> <div class="{this.barClass()}" riot-style="width: {record.progress_percentage}%"> <div class="{opts.donate_button}"></div> </div> </div> </div> </div> <div class="limited-width"> <div if="{!record.donations_prohibited && !opts.donate_button}" class="project-values"> <div class="donations-count" if="{!opts.donate_button}"> <div class="value">{record.donations_count}</div> {t.donations_count} </div> <div if="{record.progress_percentage}" class="progress-percentage"> <div class="value">{record.progress_percentage} %</div> {t.financed} </div> </div> </div> <div class="limited-width"> <div class="project-status-message" if="{record.donations_prohibited}"> {t.donations_prohibited} </div> <a target="_blank" class="button button-block show-on-mini" if="{!record.donations_prohibited}" href="{visit_url}">{t.donate}</a> <a target="_blank" class="button button-block" if="{record.donations_prohibited}" href="{visit_url}">{t.visit}</a> </div> <div class="supported-by" if="{this.isWirwunder()}"> {t.supported_by_wirwunder} </div> <div class="supported-by" if="{this.isBetterplace()}"> {t.provided_by_betterplace} </div> <div class="{this.wirwunderLogosClass()}" if="{this.isWirwunder()}"> <div class="logo"> <img src="/images/wirwunder_logo_red.svg"> </div> <div class="logo" if="{client.wirwunder_logo}"> <img riot-src="{client.wirwunder_logo}"> </div> </div> <div class="logo show-on-mini" if="{this.isBetterplace()}"> <img class="betterplace-logo" src="/images/bp-org-logo.png"> </div> <a href="{t.privacy_policy_url}" target="_blank" class="privacy-policy-link" title="{t.privacy_policy_text}">i</a> </section>', '', 'class="{opts.donate_button ? \'mini\' : \'\'}"', function(opts) {
